...
#SBATCH --output=/home/rsadve1/PROJECT/pole_transport_receiver_package/slurm/pt-train-%j.out
...
OUT_DIR="${OUT_DIR:-$REPO_DIR/results/pole_transport_weights}"
...
for req in src/data/pusch_link.py src/receivers/baselines.py src/utils/config.py src/experiments/train_pole_transport_policy.py; do
  if [ ! -f "$req" ]; then
